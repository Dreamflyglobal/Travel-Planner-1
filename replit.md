...
- **API Keys Management** — Admin panel section to securely store provider API keys (Flight/Bus/Hotel/Payment) in DB. Singleton row in `api_keys` table (`apiKeysTable` in `lib/db/src/schema/api-keys.ts`). Routes: `GET /api/admin/api-keys` returns ONLY masked values (`****1234`) + `set` flag + `source` (db/env/none) — full keys never leave the server. `POST /api/admin/api-keys` saves new values (empty string clears, missing field leaves as-is). `POST /api/admin/api-keys/test` validates a provider key is configured. Falls back to env vars (`TRIPJACK_API_KEY`, `RAPIDAPI_KEY`, `HOTELBEDS_API_KEY`, `RAZORPAY_KEY_ID`, `RAZORPAY_KEY_SECRET`) when DB row is empty. Admin-only (JWT bearer token, role check). Razorpay payment keys are read-only in UI (env-managed). Frontend: `artifacts/travel-booking/src/pages/admin/api-keys-section.tsx` — password-type inputs with show/hide toggle, masked badges, per-provider Test buttons, save/discard. Mounted in `admin/settings.tsx` between Website Settings and Notification Settings.
